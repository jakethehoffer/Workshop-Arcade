...
After a `main` push deploys through [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml), run `npm run test:live-pages` against GitHub Pages. The deploy workflow assembles a curated `_site` artifact from the static app, public docs/tooling surfaces, and `.well-known/security.txt`, then publishes it with the GitHub Pages Actions flow instead of the legacy branch-root build. The live smoke checks the deployed catalog, manifest, feed, sitemap, PWA/fallback surfaces, selected direct game pages, browser errors, mobile overflow, rendered screenshots/diagnostics, canvas nonblank evidence, and verifies the catalog does not call the GitHub API during startup. It also compares deployed shell/catalog SHA-256 content hashes (`index.html`, `websites/manifest.json`, `feed.json`, `sitemap.xml`, `app.webmanifest`, `offline.html`, `404.html`, and `robots.txt`), deployed game HTML SHA-256 content hashes, and the deployed `sw.js` `SHELL_REVISION` with local files so a Pages smoke proves the deployed shell and touched pages match the branch being verified; set `WORKSHOP_ARCADE_EXPECTED_SW_REVISION` for an intentional alternate revision, `WORKSHOP_ARCADE_SKIP_SW_REVISION=1` only for historical deploy checks, or `WORKSHOP_ARCADE_SKIP_CONTENT_HASH=1` only for intentional historical/preview content checks. Set `WORKSHOP_ARCADE_URL` for a preview deployment and `WORKSHOP_ARCADE_LIVE_SLUGS` or `WORKSHOP_ARCADE_TOUCHED_SLUGS` for the game pages touched by the release; `WORKSHOP_ARCADE_REQUIRE_LIVE_SLUGS=1` fails release verification that forgets explicit slugs. Without a slug override the smoke checks the three newest manifest entries. Each run writes `test-results/live-pages-smoke/<timestamp>/summary.json` and `test-results/live-pages-smoke/<timestamp>/report.md` for handoff evidence.
...
- [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) — GitHub Pages deployment from a curated Actions artifact with modern Pages actions, least-privilege permissions, and explicit `.well-known/security.txt` publication.
