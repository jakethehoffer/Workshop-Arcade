...
After a `main` push deploys, run `npm run test:live-pages` against GitHub Pages. It checks the deployed catalog, manifest, feed, sitemap, PWA/fallback surfaces, selected direct game pages, browser errors, mobile overflow, and verifies the catalog does not call the GitHub API during startup. It also compares the deployed `sw.js` `SHELL_REVISION` with the local `sw.js` revision so a Pages smoke proves the deployed shell matches the branch being verified; set `WORKSHOP_ARCADE_EXPECTED_SW_REVISION` for an intentional alternate revision or `WORKSHOP_ARCADE_SKIP_SW_REVISION=1` only for historical deploy checks. Set `WORKSHOP_ARCADE_URL` for a preview deployment and `WORKSHOP_ARCADE_LIVE_SLUGS` for the game pages touched by the release; without a slug override the smoke checks the three newest manifest entries. Each run writes `test-results/live-pages-smoke/<timestamp>/summary.json` for handoff evidence.
