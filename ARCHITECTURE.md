...
3. **`performance-audit`** — boots the static server on port 4173 and runs `npm run audit:perf:ci` against it. Strict mode fails if any page exceeds its publish budget (Catalog ≤ 200 KB / ≤ 22 requests; Idle Tycoon ≤ 225 KB; Lexica ≤ 300 KB; Arcade Jump ≤ 160 KB; Brick Breaker ≤ 130 KB; everything else ≤ 100 KB / ≤ 4 requests).
