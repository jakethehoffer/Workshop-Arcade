...
| **Live game smoke** | `test:live-canvas-evidence` (local Playwright fixture for live-smoke canvas aggregation) · `test:games` (local Playwright, slow — run via `npm run test:games` or `npm run test:all`) · `test:live-pages` (explicit post-deploy GitHub Pages smoke) |
...
- [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) — GitHub Pages deployment from the curated `scripts/build-pages-artifact.mjs` Actions artifact with modern Pages actions, least-privilege permissions, and explicit `.well-known/security.txt` publication.
...
8. **Commit + PR.** GitHub Actions runs the four-job validation workflow plus CodeQL; once the change reaches `main`, the Deploy Pages workflow publishes the curated static artifact.
