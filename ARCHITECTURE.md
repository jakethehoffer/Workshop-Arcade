...
| **Owned-domain launch config** | `test:owned-domain-readiness` · `test:owned-domain-rehearsal-contract` |
...
7. **Run `npm run test:games`** to confirm the Playwright smoke suite is happy with the new entry, `npm run test:publish-ready` before publishing a larger launch-QA pass, or `npm run test:owned-domain-rehearsal` before final custom-domain work.
