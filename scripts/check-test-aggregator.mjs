...
async function checkCoverage(pkg, runnerSrc) {
  ...
  const allowedExcluded = new Set(['test:github-security-settings', 'test:games', 'test:live-canvas-evidence', 'test:owned-domain-cutover-preflight', 'test:owned-domain-rehearsal', 'test:player-storage-bridge', 'test:pwa-runtime', 'test:publish-ready', 'test:realtime-progression', 'test:reduced-motion', 'test:runtime-storage', 'test:scoped-verification', 'test:live-pages', 'test:launch-evidence-current', 'test:launch-evidence-refresh', 'test:current-head-workflows', 'test:all']);
