...
The site is hosted on GitHub Pages by the repo-owned Deploy Pages workflow. Pushes to `main` assemble a curated static artifact from the app files and public docs via `scripts/build-pages-artifact.mjs`, deploy it automatically, then run the live Pages smoke against the deployed URL. The deploy smoke auto-selects touched game slugs from the push diff when possible and falls back to the newest entries otherwise.
...
- `npm run test:security-workflows` enforces that `.github/dependabot.yml` (npm + github-actions weekly updates), `.github/workflows/codeql.yml` (push + PR + weekly schedule, hardened least-privilege permissions, `security-extended` query pack, javascript-typescript analysis), and `.github/workflows/deploy-pages.yml` (shared curated GitHub Pages artifact builder, modern Pages actions, least-privilege deploy permissions, `.well-known/security.txt` publication, touched-slug selection, and post-deploy live-smoke evidence artifact) stay in place so dependency drift, inline-JS security regressions, and Pages deployment drift surface before production.
...
- `npm run test:live-pages` runs a deployed-site smoke against [the live GitHub Pages site](https://jakethehoffer.github.io/Workshop-Arcade/) by default. Deploy Pages runs it automatically after production deploys and uses `scripts/derive-live-smoke-slugs.mjs` to set touched game slugs from the push diff when possible; run it manually for preview URLs, touched-slug release checks, or independent post-deploy evidence. Set `WORKSHOP_ARCADE_URL` to check another deployment and `WORKSHOP_ARCADE_LIVE_SLUGS=pinball-foundry,prism-relay,typeforge-cipher` or `WORKSHOP_ARCADE_TOUCHED_SLUGS` to choose the direct game pages; without an override it checks the three newest manifest entries. Use `WORKSHOP_ARCADE_REQUIRE_LIVE_SLUGS=1` for release evidence that must name touched slugs. It verifies the catalog, manifest, feed, sitemap, `app.webmanifest`, `sw.js`, fallback pages, `robots.txt`, `.well-known/security.txt`, selected games, no browser errors, no mobile overflow, no catalog startup GitHub API requests, deployed shell/catalog SHA-256 content hashes, deployed game HTML SHA-256 content hashes, rendered diagnostics, screenshots, canvas nonblank evidence, and that the deployed `sw.js` `SHELL_REVISION` matches the local revision. Use `WORKSHOP_ARCADE_EXPECTED_SW_REVISION` to pin a different expected revision, `WORKSHOP_ARCADE_SKIP_SW_REVISION=1` only for intentional historical checks, or `WORKSHOP_ARCADE_SKIP_CONTENT_HASH=1` only when intentionally checking a historical/preview build whose content should not match local files, then read `test-results/live-pages-smoke/<timestamp>/summary.json` and `test-results/live-pages-smoke/<timestamp>/report.md`.
...
- `npm run audit:perf:ci` runs the same strict Pagespeed-style performance audit against `WORKSHOP_ARCADE_URL` when set, otherwise the live GitHub Pages site; CI sets the URL to its local static server.

CI runs `validate-catalog.ps1`, `npm run test:docs`, `npm run test:tools`, `npm run test:capture-recipes`, `npm run test:validator-fixtures`, `npm run test:pages-artifact`, `npm run test:a11y`, `npm run test:runtime-storage`, `npm run test:pwa-runtime`, `npm run test:live-canvas-evidence`, `npm run test:games`, `npm run audit:perf:ci`, and `npm run capture:games:ci` on every push. The Validate Catalog workflow is split into catalog/docs/a11y, game smoke, performance audit, and render capture jobs, with compact performance and render-ranking artifacts uploaded for review. The Deploy Pages workflow publishes the same branch from a curated `_site` artifact assembled by `scripts/build-pages-artifact.mjs` instead of the legacy branch-root Pages build, derives touched live-smoke slugs from the push diff when possible, then runs `npm run test:live-pages` with retry behavior and uploads `live-pages-smoke` evidence for 14 days.

See `CONTRIBUTING.md` and `docs/game-contract.md` for the full add/update/remove checklist and per-game quality contract. `ARCHITECTURE.md` walks through the script network (4 generators, 36 fast validators, 4-job CI workflow) and ends with a step-by-step "Adding a new game" recipe.
