...
The site is hosted on GitHub Pages by the repo-owned Deploy Pages workflow. Pushes to `main` assemble a curated static artifact from the app files and public docs, then deploy it automatically.
...
- `npm run test:security-workflows` enforces that `.github/dependabot.yml` (npm + github-actions weekly updates), `.github/workflows/codeql.yml` (push + PR + weekly schedule, hardened least-privilege permissions, `security-extended` query pack, javascript-typescript analysis), and `.github/workflows/deploy-pages.yml` (curated GitHub Pages artifact, modern Pages actions, least-privilege deploy permissions, and `.well-known/security.txt` publication) stay in place so dependency drift, inline-JS security regressions, and Pages deployment drift surface before production.
