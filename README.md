...
- `npm run test:current-head-workflows` verifies the remote `Validate Catalog`, `Deploy Pages`, `CodeQL`, and `Security Surfaces` workflow runs for the current clean HEAD all exist, are completed, and concluded `success`, then writes JSON/Markdown evidence under `test-results/current-head-workflows/<timestamp>/summary.json` plus `report.md`. It is intentionally excluded from `npm test` because it depends on authenticated GitHub Actions state via the `gh` CLI.
