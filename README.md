...
- `npm run test:live-pages` runs a post-deploy smoke against [the live GitHub Pages site](https://jakethehoffer.github.io/Workshop-Arcade/) by default. Set `WORKSHOP_ARCADE_URL` to check another deployment and `WORKSHOP_ARCADE_LIVE_SLUGS=lumen-lander,relay-choir,tetris` to choose the direct game pages. It verifies the catalog, manifest, feed, sitemap, selected games, no browser errors, no mobile overflow, and no catalog startup GitHub API requests.
