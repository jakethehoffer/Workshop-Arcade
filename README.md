...
- `npm run test:reduced-motion` drives a real game page in Chromium and proves `workshop-runtime.js` injects a gated `prefers-reduced-motion` reset (`style[data-workshop-reduced-motion]`): under `reduce` it collapses CSS animation/transition durations to ~0 across the corpus from one shared file, and under `no-preference` it leaves them intact so motion is only disabled for users who ask. Canvas gameplay motion is JS-driven and stays a per-game concern.
