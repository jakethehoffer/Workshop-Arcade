...
`npm test` invokes `scripts/run-fast-tests.mjs`, which auto-discovers every fast `test:*` npm script and runs them in sequence with a per-gate PASS/FAIL summary. Browser-backed runtime probes stay explicit (`npm run test:runtime-storage`, `npm run test:pwa-runtime`, `npm run test:live-canvas-evidence`) alongside the slow `npm run test:games` Playwright suite. The deployed-site smoke (`npm run test:live-pages`) is also explicit for local preview and touched-slug checks because it hits GitHub Pages or `WORKSHOP_ARCADE_URL`; the validation-gated Pages jobs inside Validate Catalog run it automatically after production deploys. The authenticated GitHub security settings check (`npm run test:github-security-settings`) stays explicit because it queries repository settings and alert APIs. `npm run test:all` adds `test:games` on top of the fast runner, `npm run test:publish-ready` runs the full local launch-QA stack and writes evidence under `test-results/publish-ready/<timestamp>/summary.json` plus `test-results/publish-ready/<timestamp>/report.md`, `npm run test:launch-evidence-refresh` refreshes publish-ready plus live Pages evidence and writes combined evidence under `test-results/launch-evidence-refresh/<timestamp>/summary.json` plus `report.md`, `npm run test:launch-evidence-current` checks the newest publish-ready and `test-results/live-pages-smoke/<timestamp>/summary.json` reports identify the current clean HEAD, `npm run test:current-head-workflows` verifies the required `Validate Catalog`, `CodeQL`, and `Security Surfaces` workflow runs plus the seven validation-gated Validate Catalog jobs completed successfully for the current clean HEAD and writes evidence under `test-results/current-head-workflows/<timestamp>/summary.json` plus `report.md`, `npm run test:owned-domain-rehearsal` proves a generated root-domain artifact under `test-results/owned-domain-rehearsal/<timestamp>/summary.json` plus `test-results/owned-domain-rehearsal/<timestamp>/report.md`, and `npm run test:owned-domain-cutover-preflight` writes final-domain readiness evidence under `test-results/owned-domain-cutover-preflight/<timestamp>/summary.json` plus `test-results/owned-domain-cutover-preflight/<timestamp>/report.md`. These slow evidence reports, live Pages smoke, and CI-uploaded `game-smoke-summary`, `performance-audit`, and `render-ranking` artifacts include source revision provenance: branch, commit, dirty state, manifest game count, and newest manifest slugs.
...
npm run test:player-data-controls
...
- `npm run test:security-workflows` enforces that every external GitHub Action use is pinned to a full immutable commit SHA with a same-line `vX.Y.Z` release comment, and that one consistent release is used per action repository. It also keeps `.github/dependabot.yml` (npm + github-actions weekly updates), `.github/workflows/codeql.yml` (push + PR + weekly schedule, hardened least-privilege permissions, `security-extended` query pack, javascript-typescript analysis), `.github/workflows/validate-catalog.yml` (four validation jobs followed by validation-gated Pages build, deploy, and live smoke jobs with no independent deploy workflow), and `.github/workflows/security-surfaces.yml` (the Security Surfaces workflow for GitHub-native repository settings drift) in place so dependency drift, action supply-chain drift, inline-JS security regressions, Pages deployment drift, and security-settings drift surface before production.
...
- `npm run test:launch-evidence-refresh` is the post-commit clean-HEAD evidence command: it requires a clean worktree, runs `npm run test:publish-ready`, `npm run test:live-pages`, and `npm run test:launch-evidence-current` in order, then writes combined JSON/Markdown evidence under `test-results/launch-evidence-refresh/<timestamp>/summary.json` plus `report.md`. It is intentionally excluded from `npm test` because it runs the slow browser/performance-backed launch stack.
...
- `npm run test:current-head-workflows` verifies the remote `Validate Catalog`, `CodeQL`, and `Security Surfaces` workflow runs for the current clean HEAD, rejects a standalone `Deploy Pages` run, and inspects the Validate Catalog jobs `Catalog, docs, and accessibility`, `Game smoke tests`, `Performance audit`, `Render capture`, `Build static artifact`, `Deploy`, and `Live Pages smoke`. Its `requiredValidateJobs`, `validateJobs`, and `deploymentGate` JSON fields prove the build started only after all four validations completed, followed by deploy and live smoke, then it writes evidence under `test-results/current-head-workflows/<timestamp>/summary.json` plus `report.md`. It is intentionally excluded from `npm test` because it depends on authenticated GitHub Actions state via the `gh` CLI.
...
- `npm run test:playwright-harness` verifies the shared Playwright helper keeps reusable desktop and mobile browser contexts, clears local/session storage before each game document, recognizes only `ERR_NO_BUFFER_SPACE` and `ERR_INSUFFICIENT_RESOURCES`, performs one bounded retry after context recreation, and emits the exact `transportRetryCount` / `transportRetries` evidence shape.
...
- `npm run test:csp` verifies the `<meta http-equiv="Content-Security-Policy">` in `index.html` declares every critical directive and authorizes executable inline JavaScript with exact SHA-256 sources instead of `'unsafe-inline'`. It rejects stale or extra hashes, nonces, remote script origins, `'unsafe-eval'`, inline event handlers, and restored script `'unsafe-inline'`; inert `application/ld+json` blocks are intentionally excluded. Run `npm run build:csp` after directly editing executable JavaScript in `index.html`; `validate-catalog.ps1 -Fix` already does this after changing `FALLBACK_GAMES`. Inline catalog CSS remains covered by `style-src 'unsafe-inline'`. The same gate also requires every manifest game page to retain its existing tighter generated CSP before the first script.
...
- `npm run test:player-data-controls` locks in the footer Player data modal: visible local counts for favorites, recent plays, feedback drafts, and sandbox-player saves; selective clear actions; a two-activation full `localStorage` reset; local-only privacy copy; and integration with the shared Escape/focus-trap behavior. The browser smoke also proves the controls remove the intended keys and refresh Continue/Favorites/draft UI immediately.
...
- `npm run test:player-storage-bridge` drives the real catalog player in Chromium and proves saves persist for sandboxed games: the player seeds each game's saved entries through a `#wa-storage=` fragment, `workshop-runtime.js` posts writes back over `postMessage`, the catalog mirrors them under `workshop-arcade:game:<slug>:` keys, `clear()` stays scoped to the active game, direct game loads keep native storage with no mirror writes, and values survive a full reload + player reopen.
- `npm run test:reduced-motion` drives a real game page in Chromium and proves `workshop-runtime.js` injects a gated `prefers-reduced-motion` reset (`style[data-workshop-reduced-motion]`): under `reduce` it collapses CSS animation/transition durations to ~0 across the corpus from one shared file, and under `no-preference` it leaves them intact so motion is only disabled for users who ask. It also verifies that "motion-aware" games — those that gate decorative canvas motion (screen shake, particles) behind a per-game reduced-motion read — report the live `reducedMotion` state in `render_game_to_text()` under both settings, so the canvas-motion gating is testable rather than visual-only. (Core gameplay motion stays intentionally untouched.)
- `npm run test:realtime-progression` drives time-driven games in Chromium through real elapsed wall-clock time only (never calling `advanceTime`) and asserts their time-driven diagnostic (e.g. the beat clock) actually climbs. This catches the "frozen game" class where `step()`/`update()` runs only inside `advanceTime` — so the game passes smoke/capture (which drive via `advanceTime`) while being frozen for real players, as `signal-loom` and `tempo-forge` both were before they gained real `requestAnimationFrame` loops.
- `npm run test:scoped-verification` proves the scoped verification fast path stays honest: `npm run test:games -- --slug <a,b>` and `npm run capture:games -- --slug <a,b>` cover exactly the requested games and write `scope`-marked summaries, unknown slugs are refused instead of silently running the full catalog, and `capture:games:ci` refuses `--slug` so CI rendered-quality evidence always means the full catalog passed.
...
- `npm run test:games` starts a local static server, verifies the catalog player modal, player shelves, suggestion draft URL flow, and opens every manifest game on desktop and mobile viewports. The direct-game sweep uses reusable desktop and mobile browser contexts and allows one bounded retry only for the two recognized Windows transport resource errors after recreating the affected context. It writes `test-results/smoke-games/<timestamp>/summary.json` with source revision provenance plus `transportRetryCount` and `transportRetries`.
- `npm run capture:games:ci` runs the rendered-quality harness in strict mode and fails if any captured surface scores above 0. It uses the same reusable desktop and mobile browser contexts and one bounded retry policy, while keeping the contact-sheet capture isolated. For optional local review, `npm run capture:games` writes the same ranked contact sheet plus `test-results/render-ranking/<timestamp>/summary.json` under `test-results/render-ranking/<timestamp>/` without CI strictness; both the summary and contact sheet include source revision provenance. The summary records pass/fail status, expected/captured surface counts, last capture phase, `transportRetryCount`, and `transportRetries`; unexpected early aborts still write a failure summary, with a partial contact sheet when at least one surface was captured.
...
- `npm run test:publish-ready` runs the local launch-QA stack in order: catalog validation, `npm test`, runtime probes, game smoke, strict render capture, local performance audit, and `git diff --check`. It stops on the first failure but still writes `summary.json` and `report.md` under `test-results/publish-ready/<timestamp>/`, including source revision provenance for the exact branch/commit/worktree and manifest it checked. After pushing, use `npm run test:current-head-workflows` once the required Actions runs finish so the local launch handoff also has pass/fail evidence for the current clean HEAD's remote workflow status.
...
See `CONTRIBUTING.md` and `docs/game-contract.md` for the existing-game maintenance checklist and per-game quality contract. `ARCHITECTURE.md` walks through the script network (5 generators, 47 fast validators, 4 parallel validation jobs plus 3 gated deployment jobs), content-freeze policy, and validation-gated deployment flow.

## License & Security Reports

Workshop Arcade is distributed under the [MIT License](LICENSE) — fork, modify, and remix freely as long as the copyright notice travels along. Security disclosures follow [RFC 9116](.well-known/security.txt) and the [SECURITY.md disclosure policy](SECURITY.md): please open a [private GitHub security advisory](https://github.com/jakethehoffer/Workshop-Arcade/security/advisories/new) for anything that could let a malicious game submission compromise visitors. Validate Catalog's gated Pages artifact publishes the `.well-known/security.txt` surface assembled by `scripts/build-pages-artifact.mjs`. [CodeQL](.github/workflows/codeql.yml) scans the inline catalog JavaScript + the Node tooling on every push and weekly. Every external workflow action is pinned to an immutable commit SHA, while [Dependabot](.github/dependabot.yml) keeps the documented releases and pins current. Informal credits live in [humans.txt](humans.txt).
