...
- `npm run test:pwa-install-budget` statically sums the service-worker install payload (`sw.js`, install shell assets, the small newest-cover pre-cache set, and local PWA manifest icons) against the Catalog performance budget so offline support cannot silently exceed the publish contract.
