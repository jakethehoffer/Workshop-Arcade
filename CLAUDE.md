...
npm run test:landscape-playfield
...
- `npm run test:landscape-playfield` is the phone-landscape blackout gate: it loads every game at 844x390 and requires the playfield canvas to start inside the first screen with at least 64 visible pixels. Every other browser gate runs at 1280x820 and 390x844, both tall enough to hide a width-only breakpoint serving the tall stacked layout to a short viewport. It runs in the Validate Catalog "Game smoke tests" job, so a landscape regression blocks the deploy gate. Run it after any change to a game's responsive CSS. A game whose board is DOM-rendered has no playfield canvas and is skipped, but it can opt in by tagging the surfaces a sideways player cannot do without with `data-landscape-essential="<name>"`; every tagged element must fit *entirely* inside the first landscape screen, because half a digit pad is not a usable digit pad. `wordle`, `volt-sudoku` and `cipher-rooms` tag their board and their on-screen keypad, `echo-mimic` tags its four pads, and `memory-match` tags its card board. `rail-yard-relay` tags the whole yard, clock/status and route/touch/game controls, since moving trains require the complete track and controls together. `prism-relay` tags its full light puzzle, move budget, rotation preview/receivers, status and controls. `2048` tags its whole sixteen-tile board, score, status and controls; short landscape screens keep them together, with a scrollable help card.
